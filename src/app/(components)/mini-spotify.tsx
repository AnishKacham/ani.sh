...
export default async function MiniSpotify() {
  try {
    ...
    */
    ...
    return (
      <div className="flex flex-row items-center w-36 rounded-md button gap-1 bg-slate-700">
        <div className="w-[60px] h-[40px] rounded-l-md bg-slate-600 flex justify-center items-center">
          <Image src={Spotify} alt="spotify" width={22} height={22} />
        </div>
        <div className="w-full flex flex-col gap-1.5 pr-1.5 text-center">
          <span className="font-extrabold text-sm">TLS ERR.</span>
        </div>
      </div>
    );
  }
